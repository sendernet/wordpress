...
class Sender_Carts
{
    ...
    public function senderConvertCart($orderId)
    {
        ...
        if (in_array($orderPost->post_status, Sender_Helper::ORDER_NOT_PAID_STATUSES)) {
            ...
            add_action('sender_cart_update_status_action', [$this, 'addStatusCartUpdateScript']);
            do_action('sender_cart_update_status_action', $cartStatus);
            return;
        }
        ...
        if (!empty($items)) {
            ...
            if (wp_doing_ajax()) {
                ...
                add_action('wp_head', function() use ($cartData) {
                    $this->addTrackCartScript($cartData);
                });
            }
        }
    }
